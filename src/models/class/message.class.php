...
class Message extends Database
{
  ...
  function getMessagesSubject(): array | bool {
    $getSubjects = $this->pdo->prepare('SELECT messages.* , users.id_u , users.pseudo, users.avatar FROM messages INNER JOIN users WHERE messages.id_users = users.id_u ORDER BY date DESC');
    $getSubjects->execute();
